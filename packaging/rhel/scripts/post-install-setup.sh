#!/bin/bash
# Auditcore Post-Installation Setup Script
# Run this after RPM installation to complete setup

set -e

echo "======================================"
echo " Auditcore Post-Install Setup"
echo "======================================"
echo ""
...
if ! systemctl list-unit-files | grep -q ciso-assistant-backend; then
    echo "ERROR: Auditcore services not found. Please install the RPM first."
    exit 1
fi
...
echo "----------------------------"
read -p "Start Auditcore services now? (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
