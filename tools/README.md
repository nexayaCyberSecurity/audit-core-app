...
The `prepare_framework_v2.py` script can be used to create an Excel file with the base structure of a Auditcore framework in v2 format.

The `convert_library_v2.py` script can be used to transform an Excel file to a Auditcore library.
...
This file can be converted with `convert_library_v2.py` and imported into Auditcore to see how it looks.
...
The resulting YAML file adheres to the Auditcore schema and can be directly imported into the platform.
...
If you want to update a framework, don't forget to increment the version number in the `version` field of the `library_meta` sheet before converting it to YAML. Otherwise, if the version number of the framework file is lower than or equal to the version number of the old version of your already imported framework in Auditcore, your framework will not be updated and Auditcore will not suggest to update the framework in your loaded libraries.
