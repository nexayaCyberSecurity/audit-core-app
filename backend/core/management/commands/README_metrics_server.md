...
The `metrics_server` management command runs a standalone HTTP server that exposes Prometheus metrics on a separate port from the main Auditcore application. This approach provides better security and performance by isolating metrics collection from the main application.
