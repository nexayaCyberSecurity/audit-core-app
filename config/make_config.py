    ...
    if enable_dispatcher:
        config["kafka_dispatcher"] = {
            ...
            "authentication": questionary.select(
                "How would you like to authenticate requests to the Auditcore API using the dispatcher (credentials/token)?",
        ...
        if config["kafka_dispatcher"]["authentication"] == "credentials":
            config["kafka_dispatcher"]["credentials"] = {
                "user_email": questionary.text(
                    "Enter the email of the Auditcore user account"
                ).ask(),
            ...
            config["kafka_dispatcher"]["auto_renew_session"] = questionary.confirm(
                "Enable silent reauthentication to the Auditcore API on session expiry?",
                default=True,
            ).ask()
        ...
        use_s3 = questionary.confirm(
            "Would you like to connect a S3 bucket to the dispatcher? This can be used e.g. to upload files to the Auditcore backend.",
            default=True,
        ).ask()
...
def main():
    print("[blue]Auditcore Docker Compose Configuration Builder[/blue]")
