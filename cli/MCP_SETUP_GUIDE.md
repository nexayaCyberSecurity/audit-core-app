# Auditcore MCP Setup Guide

This guide explains how to connect your AI assistant to Auditcore using the Model Context Protocol (MCP). Once set up, you'll be able to ask your AI to create risk assessments, manage compliance audits, track controls, and much more - all through natural conversation.

## What is MCP?

MCP (Model Context Protocol) allows AI assistants like Claude to interact with external tools and services. Think of it as giving your AI a set of capabilities to read and write data in Auditcore.

The Auditcore MCP server provides **80+ tools** covering:
...
2. **Network control** - All API calls to Auditcore go through your local machine. You have full visibility and control over network traffic, and can use your existing firewall rules and proxies.
...
5. **Works offline** - The MCP server itself runs locally. Only the actual Auditcore API calls require network access.

## Step 0: Get the MCP Server Code

The MCP server code is included in the Auditcore repository. You need to download it to your machine first.
...
1. **Auditcore running** - Either locally or on a server (can be the same machine or a remote server)
...
You need a token to authenticate the MCP server with Auditcore:

1. Log in to Auditcore
...
Navigate to the `cli` folder in your Auditcore installation:
...
> "What folders exist in Auditcore?"

If configured correctly, Claude will use the MCP tools to query your Auditcore instance.
...
> "List all risk assessments in Auditcore"
...
### Step 2: Add the Auditcore server
...
- Make sure Auditcore is running
...
- **Auditcore Documentation:** https://intuitem.gitbook.io
...
|----------|----------|---------|-------------|
| `TOKEN` | Yes | - | Personal Access Token from Auditcore |
| `API_URL` | No | `http://localhost:8000/api` | Auditcore API endpoint |
